        ...
        for transition in self.transitions:
            ...
            if from_state == self.current_state:
                ...
                if match:
                    ...
                    if options.debug:
                        print >>sys.stderr, "# FSM: executing (%s -> %s) \
                                for line '%s'" % (from_state, to_state, input)
                    callback(match)
                    return
...
class Doxypy(object):
    ...
    def __init__(self):
        ...
        transitions = [
            ...
            # other lines
            ["FILEHEAD", "FILEHEAD",
                self.empty_re.search, self.appendFileheadLine],
            ["FILEHEAD", "FILEHEAD",
                self.hashline_re.search, self.appendFileheadLine],
            ["FILEHEAD", "FILEHEAD",
                self.importline_re.search, self.appendFileheadLine],
            ["FILEHEAD", "DEFCLASS",
                self.defclass_re.search, self.resetCommentSearch],
            ...
            # other lines
            ["DEFCLASS", "DEFCLASS",
                self.empty_re.search, self.appendDefclassLine],
            ["DEFCLASS", "DEFCLASS",
                self.defclass_re.search, self.resetCommentSearch],
    ...
    def __flushBuffer(self):
        ...
        if self.output:
            try:
                ...
                # stdout prematurely upon usage of INPUT_FILTER,
                # INLINE_SOURCES and FILTER_SOURCE_FILES.
                pass
        self.output = []
    ...
    def appendCommentLine(self, match):
        ...
        or (from_state == "FILEHEAD" and to_state == "FILEHEAD"):
            ...
            self.comment.append(line[
                    line.find(activeCommentDelim)+len(activeCommentDelim):
                    line.rfind(activeCommentDelim)])

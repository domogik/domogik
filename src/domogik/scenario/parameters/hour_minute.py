...
class HourMinuteParameter(AbstractParameter):
    """ This parameter  simply provides a hour + minute entry
    """

    def __init__(self, log = None, trigger = None):
        AbstractParameter.__init__(self, log, trigger)
        self.set_type("time")
        self.add_expected_entry("hour", "number", "Hour")
        self.add_expected_entry("minute", "number", "Minute")
        self.expr = None
        self._event = Event()
        self._fetch_thread = Thread(target=self._check,name="HourMinuteParameter.check")
        self._fetch_thread.start()

    def _check(self):
        """ This method will check each minute if the hour:minute is the current hour
        """
    ...
    def fill(self, params):
        ...
        if "hour" in params and "minute" in params:
            cron_expr = "{0} {1} * * *".format(minute, hour)
            self.expr = CronExpression(cron_expr)
        else:
            self.expr = None

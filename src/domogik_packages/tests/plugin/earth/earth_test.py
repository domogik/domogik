...
Make tests for the XPL subsystem
...
class PluginTestCase(unittest.TestCase):
    ...
    def query_many(self, key, testmsg, dictkeys=[], dictkeyvals={}, retry=10):
        ...
        while ((left >= 0) and (res != True)):
            res = self.query(key, testmsg, dictkeys, dictkeyvals, timeout=20)
            left = left -1
        return res
...
class MoonPhasesTestCase(PluginTestCase):
    ...
    def test_120_stop_full_moon(self):
        message = XplMessage()
        message.set_type("xpl-cmnd")
        message.set_schema("earth.basic")
        message.add_data({"action" :  "stop"})
    ...
    def test_121_resume_full_moon(self):
        message = XplMessage()
        message.set_type("xpl-cmnd")
        message.set_schema("earth.basic")
        message.add_data({"action" :  "resume"})

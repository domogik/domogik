...
class serialHandler(threading.Thread):
    ...
    def send(self, plcbus_frame):
        ...
        if (int(plcbus_frame[8:10], 16) >> 5) & 1: #ACK pulse bit set to 1
            # wait a bit, (in the spec it is written 400ms, in a forum it is
            # stated that it should be a typo and should be 40ms)
            # TODO: Extract 0.41 to a class-level constant w/ a meaningful name
            time.sleep(0.41)
            ACK_received = 0
            ...
            while 1:
                ...
                while 1:
                    ...
                    # put message in _received_queue
                    # in order to be sent on the xPL network
                    print "recu " + hexlify(message)
                    # check if the received frame is the waited ACK
                    if(message and self._is_ack(message, plcbus_frame)):
                        ACK_received=1
                        # print "ACK received"

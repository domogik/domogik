...
class GenericTestCase(unittest.TestCase):
    ...
    def remove_all_device_feature_associations(self):
    ...
    def remove_all_ui_item_config(self):
...
class AreaTestCase(GenericTestCase):
    ...
    def test_empty_list(self):
        assert len(db.list_areas()) == 0

    def test_add(self):
        try:
            db.add_area(None, None)
        ...
        assert db.list_areas()[0].name == 'area0'
        assert len(db.get_all_rooms_of_area(area0.id)) == 0

    def test_update(self):
        area = db.add_area('area0','description 0')
        area_u = db.update_area(area.id, 'area1','description 1')
        assert area_u.name == 'area1'
        assert area_u.description == 'description 1'

    def test_fetch_information(self):
        area = db.add_area('area0','description 0')
        area0 = db.get_area_by_name('Area0')
        assert area0.name == 'area0', 'area0 not found'
        basement = db.add_area('Basement')
        first_floor = db.add_area('First floor')
        db.add_room(r_name='Bedroom1', r_area_id=first_floor.id)
        db.add_room(r_name='Bedroom2', r_area_id=first_floor.id)
        db.add_room(r_name='Kitchen', r_area_id=basement.id)
        db.add_room(r_name='Bathroom', r_area_id=basement.id)
        db.add_room(r_name='Lounge', r_area_id=basement.id)
        for area in db.list_areas():
...
class RoomTestCase(GenericTestCase):
    ...
    def test_add(self):
        try:
            db.add_room(r_name='foo', r_area_id=99999999999, r_description='foo')
            TestCase.fail(self, "An exception should have been raised : area id does not exist")
        except DbHelperException:
            pass
        room = db.add_room(r_name='my_room', r_area_id=None, r_description='my_description')
        print(room)
        area1 = db.add_area('area1','description 1')
        area2 = db.add_area('area2','description 2')
        room1 = db.add_room(r_name='room1', r_description='description 1', r_area_id=area1.id)
        assert room1.name == 'room1'
        assert room1.description == 'description 1'
        assert room1.area_id == area1.id
        room2 = db.add_room(r_name='room2', r_description='description 2', r_area_id=area1.id)
        room3 = db.add_room(r_name='room3', r_description='description 3', r_area_id=area2.id)
        assert len(db.list_rooms()) == 4
        room4 = db.add_room(r_name='room4')

    def test_update(self):
        area1 = db.add_area('area1','description 1')
        area2 = db.add_area('area2','description 2')
        room = db.add_room(r_name='room1', r_description='description 1', r_area_id=area1.id)
        try:
            db.update_room(room.id, r_name='room2', r_description='description 2', r_area_id=99999999999)
            TestCase.fail(self, "An exception should have been raised : area id does not exist")
        except DbHelperException:
            pass
        room_u = db.update_room(room.id, r_name='room2', r_description='description 2', r_area_id=area2.id)
        assert room_u.name == 'room2'
        assert room_u.description == 'description 2'
        assert room_u.area_id == area2.id
        room_u = db.update_room(room.id, r_area_id='')
    ...
    def test_del(self):
        ...
        room3 = db.add_room(r_name='room3', r_description='description 3', r_area_id=area2.id)
        room1_id = room1.id
        room_deleted = db.del_room(room1.id)
        assert not self.has_item(db.list_rooms(), ['room1'])
        assert self.has_item(db.list_rooms(), ['room2', 'room3'])
        assert room_deleted.id == room1_id
        assert len(db.list_device_feature_associations_by_room_id(room_deleted.id)) == 0
        try:
            db.del_room(12345678910)
    ...
    def test_list_and_get(self):
        area1 = db.add_area('area1','description 1')
        room1 = db.add_room(r_name='room1', r_description='description 1', r_area_id=area1.id)
        assert db.get_room_by_name('Room1').name == 'room1'

        area2 = db.add_area('area2','description 2')
        room2 = db.add_room(r_name='room2', r_description='description 2', r_area_id=area1.id)
        room3 = db.add_room(r_name='room3', r_description='description 3',r_area_id=area2.id)
        assert len(db.get_all_rooms_of_area(area1.id)) == 2
        assert len(db.get_all_rooms_of_area(area2.id)) == 1
...
class DeviceFeatureAssociationTestCase(GenericTestCase):
    ...
    def setUp(self):
        self.remove_all_device_usages()
        self.remove_all_device_feature_associations()
        self.remove_all_device_technologies()
    ...
    def test_empty_list(self):
        assert len(db.list_device_feature_associations()) == 0
    ...
    def test_add_get_list(self):
        ...
        du2 = db.add_device_usage('lamp', 'Lamp')
        ...
        df_list = db.list_device_feature_by_device_feature_model_id(af1.id)
    ...
    def test_del(self):
        ...
        dfa2 = db.add_device_feature_association(d_feature_id=df2.id, d_place_id=room1.id, d_place_type='room')
        dfa3 = db.add_device_feature_association(d_feature_id=df2.id, d_place_id=area2.id, d_place_type='area')
        df3 = db.get_device_feature(device3.id, afm2.id)
        dfa4 = db.add_device_feature_association(d_feature_id=df3.id, d_place_id=area1.id, d_place_type='area')
        dfa5 = db.add_device_feature_association(d_feature_id=df3.id, d_place_type='house')
        assert len(db.list_device_feature_associations()) == 5
        dfa = db.del_device_feature_association(dfa1.id)
        assert dfa.id == dfa1.id
        assert len(db.list_device_feature_associations()) == 4
        assert len(db.list_device_feature_associations_by_room_id(room1.id)) == 1
        assert len(db.list_device_feature_associations_by_area_id(area1.id)) == 1
        assert len(db.del_device_feature_association_by_device_feature_id(df3.id)) == 2
        assert len(db.del_device_feature_association_by_place(area2.id, 'area')) == 1
        assert len(db.list_device_feature_associations()) == 1
...
class UIItemConfigTestCase(GenericTestCase):
    ...
    def setUp(self):
        self.remove_all_ui_item_config()

    def tearDown(self):
        self.remove_all_ui_item_config()

    def test_empty_list(self):
        assert len(db.list_all_ui_item_config()) == 0

    def test_add(self):
        ui_config = db.set_ui_item_config('area', 2, 'icon', 'basement')
        assert ui_config.name == 'area'
        assert ui_config.reference == '2'
        assert ui_config.key == 'icon'
        assert ui_config.value == 'basement'
        print(ui_config)
        db.set_ui_item_config('room', 1, 'icon', 'kitchen')
        db.set_ui_item_config('room', 4, 'icon', 'bathroom')
        db.set_ui_item_config('room', 4, 'param_r2', 'value_r2')
        ui_config_list_all = db.list_all_ui_item_config()
        assert len(ui_config_list_all) == 4, len(ui_config_list_all)
        assert len(db.list_ui_item_config_by_key(ui_item_name='room', ui_item_key='icon')) == 2
        ui_config_list_r = db.list_ui_item_config_by_ref(ui_item_name='room', ui_item_reference=4)
        ...
        uic = db.get_ui_item_config('room', 4, 'param_r2')
        assert uic.value == 'value_r2'
        uic = db.get_ui_item_config('area', 2, 'icon')
        assert uic.value == 'basement'
        assert db.get_ui_item_config('foo', 13, 'param_a1') is None

    def test_update(self):
        db.set_ui_item_config('area', 1, 'icon', 'basement')
        db.set_ui_item_config('room', 1, 'icon', 'bathroom')
        db.set_ui_item_config('room', 1, 'param_r2', 'value_r2')
        uic = db.set_ui_item_config('area', 1, 'icon', 'first_floor')
        assert uic.value == 'first_floor'
        db.set_ui_item_config('room', 1, 'icon', 'kitchen')
        assert db.get_ui_item_config('room', 1, 'icon').value == 'kitchen'

    def test_list_and_get(self):
        db.set_ui_item_config('area', 1, 'icon', 'basement')
        db.set_ui_item_config('room', 1, 'icon', 'bathroom')
        db.set_ui_item_config('room', 1, 'param_r2', 'value_r2')
        db.set_ui_item_config('room', 2, 'icon', 'kitchen')
        assert len(db.list_all_ui_item_config()) == 4
        assert len(db.list_ui_item_config(ui_item_name='room')) == 3
        assert len(db.list_ui_item_config_by_ref(ui_item_name='room', ui_item_reference=1)) == 2
        item=db.get_ui_item_config(ui_item_name='room', ui_item_reference=2, ui_item_key='icon')
    ...
    def test_del(self):
        ...
        db.set_ui_item_config('room', 1, 'icon', 'bathroom')
        db.set_ui_item_config('room', 1, 'param_r2', 'value_r2')
        db.del_ui_item_config('area', 1, 'icon')
        assert len(db.list_all_ui_item_config()) == 2
        assert db.get_ui_item_config('room', 1, 'icon') is not None
        db.del_ui_item_config(ui_item_name='room', ui_item_reference=1)
        assert len(db.list_all_ui_item_config()) == 0
        assert db.get_ui_item_config('area', 1, 'icon') is None
        db.set_ui_item_config('area', 2, 'icon', 'first_floor')
        db.set_ui_item_config('area', 2, 'pa1', 'va1')
        db.set_ui_item_config('room', 2, 'icon', 'kitchen')
        db.set_ui_item_config('room', 2, 'pr1', 'vr1')
        db.del_ui_item_config(ui_item_name='area', ui_item_reference=2)
        assert len(db.list_ui_item_config(ui_item_name='area')) == 0
        db.del_ui_item_config(ui_item_name='room', ui_item_key='icon')
        ui_item_list = db.list_ui_item_config(ui_item_name='room')
